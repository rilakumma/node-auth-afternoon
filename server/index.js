...
function fetchAuth0AccessToken(userInfoRes){
...
function fetchGitHubAccessToken(auth0AccessTokenRes){
  const options = {
    headers: {
      authorization: `Bearer ${auth0AccessTokenRes.data.access_token}`
    }
  };
  return axios.get(`https://${process.env.REACT_APP_AUTH0_DOMAIN}/api/v2/users/${req.session.user.sub}`, options);
}

function setGitTokenToSession(gitHubAccessTokenRes){
  req.session.gitHubAccessToken = gitHubAccessTokenRes.data.identities[0].access_token;
  res.redirect('/');
}
